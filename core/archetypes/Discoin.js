    ...
    this.rest = (METHOD, route, payload, query) => {
      ...
      return new Promise((resolve, reject) => {
        (request[METHOD])({ url: `${ROOT}/${route}${query ? `?s=${encodeURIComponent(query)}` : ""}`, headers: { Authorization: `Bearer ${this.token}` }, json: payload }, (err, res, body) => {
          if (err || rescodes.indexOf(res.statusCode) === -1) return reject(`[${res.statusCode}] :: API failure`);
          if (![200, 201].includes(res.statusCode)) return reject({ body, request: `${METHOD.toUpperCase()}: /${route}`, payload });
          return resolve(body);
        });
      });
    };
