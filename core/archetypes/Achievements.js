...
class AchievementsManager extends EventEmitter {
  ...
  progress(ach, user) {
    return new Promise(async (resolve) => {
      ...
      const statistics = (await DB.control.get(user.id))?.data.statistics;
      const achiev = await this.get(ach);
      const conditions = achiev.condition?.split(">=") || 0;
      const goal = conditions?.[1] || 1;
      const current = eval(`try{${conditions[0] || 0}}catch(err){0}`);
  ...
  check(userData, beau, awardRightAway) {
    return new Promise(async (resolve, reject) => {
      ...
      const res = await DB.achievements.find({}, {
        ...
        a.map(async (achiev) => {
          const user = userData;
          const revealed = userData.modules.level >= achiev.reveal_level && (this.has(userData.id, achiev.achiev_requisites) || true) && !!eval(achiev.reveal_requisites);
          const C1 = eval(`try{${achiev.condition}}catch(err){false}`);
          let C2;
          if (achiev.advanced_conditions?.length > 0) C2 = achiev.advanced_conditions.every((acv) => { const user = userData; return eval(`try{${acv}}catch(err){false}`); });
          else C2 = true;
          const awarded = userData.modules.achievements.find((a) => a.id == achiev.id)?.unlocked;
