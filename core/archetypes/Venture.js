...
Venture: class Venture {
    ...
    constructor (player,time,landscape){
        ...
        let event_count = Math.round((time*60) / TIME_SLICE);
        let ev_i =0
        while(ev_i++ < event_count){            
            const eligible_events = VENTURE_EVENTS.filter(ev=>{
                return (ev.timemax >= (ev_i*60) && ev.timemin <= (ev_i*60)) &&
                (ev.onetime ? !stack.includes(ev.id) : true) &&
                (ev.landscape ? ev.landscape === this.landscape : true);                 
            });
            let odds = eligible_events.map(ev=>ev.odds);
    ...
    constructor(Venture){
        ...
        this.journal = this.log.map((idx,i)=>{
            ...
            Object.keys(EVT).forEach(ky=>{                                
                if(detail.res){
                    let {loss,spoil} = concatenateSpoils(EVT,ky);
            ...
            Object.keys(detail.payQuery).forEach(ky=>{
                let {loss,spoil} = concatenateSpoils(detail.payQuery,ky);
                losses= losses.concat(spoil,loss);
            })
          
            return {text: detail.text, losses, spoils, moment: i+1}
...
function eventProcessor(event,Venture){
    let {supply,journey,prize,pay} = Venture;
    ...
    if (event.condition){
        ...
        if (Criteria == 'rubines'        && supply.rubines >= Items)
        (res= true) && (payQuery.rubines = Items); 
        
        else if (Criteria == 'item'      && supply.item.find(i=>i.id == Items && i.count > 0))
        (res= true) && (payQuery.item = Items); 
        
        else if (Criteria == 'level'     && this.player.level >= Items)
        (res= true);
        
        else if (Criteria == 'jades'     && supply.jades >= Items)
        (res= true) && (payQuery.jades = Items);
        
        else if (Criteria == 'items-or'  && Items.split(',').some(itm=> supply.item.find(i=> i.id == itm && i.count>0 )))
        (res= true) && (payQuery.items = supply.item.find(x=> x==Items.split(',') ));
        
        else if (Criteria == 'items-all' && Items.split(',').every(itm=> supply.item.find(i=> i.id == itm && i.count>0 )))
        (res= true) && (payQuery.items = Items.split(','));
        
        else if (Criteria == 'sapphires' && supply.sapphires >= Items)
        (res= true) && (payQuery.sapphires = Items); 
        
        else if (Criteria == 'encounter' && journey.includes(Items))
        (res= true);
        
        
        if(text.includes("|")){
            if(!res){
                let [pay,what] = event.fail.split(':')
