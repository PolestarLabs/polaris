...
checks:function(msg,UD){
  ...
  for (i in Checks){
    if(msg.match(Checks[i].regex)) {
      ...
      DB.users.set(UD.id,{$inc:{['modules.statistics.buster.'+Checks[i].category]:1}});
    }
  }
  return penalty;
...
run: async function(message,SD,UD,CD){
  ...
  //targetto = (await DB.users.find({'modules.statistics.healthIndex':{$exists:true}, blacklisted:{$exists:false}},{_id:-1,id:1}).sort({'modules.statistics.healthIndex':1}).limit(25)).map(x=>x.id);
  ...
  if(UD.modules.statistics&&UD.modules.statistics.healthIndex<-1000){
    ...
    DB.users.set(message.author.id,{$inc:{'modules.statistics.healthIndex':-penalty}});
    DB.servers.set(message.guild.id,{$inc:{'modules.statistics.healthIndex':-penalty}});
   // console.error("MATCH TRUE:".red+(msg.bgBlue))
    
  }else{
    let positive = 1 + Math.round(msg.length/10||0)
    DB.users.set(message.author.id,{$inc:{'modules.statistics.healthIndex':+positive}});
    DB.servers.set(message.guild.id,{$inc:{'modules.statistics.healthIndex':+positive}});
