const init = async function (msg,args){
    ...
    if(subcommand == 'add'){
        ...
        let targetRole = rolefind(4);
        let temp = ((Number(arg2)||0)*60000)||null;

     
        
        if(!targetRole) return "Role not found";
        let short =  arg3 || role.name.split(' ')[0].toLowerCase();
        if(roleMarket.find(entry=>entry.role == targetRole.id )) return "Role already here";
        await DB.paidroles.new({server: msg.guild.id, role: targetRole.id, price: Number(arg1) || 1000, short , temp });
        return {
            embed:{
