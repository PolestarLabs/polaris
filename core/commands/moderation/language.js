...
async function saveLanguage(langTo,scope,interaction,P){     
  if (langTo) {
      ...
      let scopeChannel = PLX.getChannel(scope);
      console.log({scope});
      scopeChannel ??= PLX.getChannel(interaction.message?.channel?.id||"000");

      if (scopeChannel) {        

        await DB.channels.set(scopeChannel.id, { "modules.LANGUAGE": langTo.iso });
        scopeChannel.LANG = langTo.iso;
        P.lngs = [langTo.iso]
...
const init = async (msg, args) => {
  ...
  const scope = msg.channelMentions?.[0] || args[1];
  console.log(args);
  ...
  if (language) {
    ...
    return await saveLanguage(langTo, scope ||( args[1]==='-c'?'channel':'server'),msg,P);
  }
...
module.exports = {
  ...
  slashable: true,
  slashOptions:{
    description: "🌍 Change the language I speak",
    global: false,
    args: ["language","scope"],
    ...
    options: [
      {
        ...
        type: 3,
        choices: Languages.i18n.map(o=>({name:o.name, value: o.iso})),
        required: true,
      },
      {
        name: "scope",
        description: "Where to apply this language setting, leave blank for server.",
        type: 7,
        required: false,
      }
    ]
