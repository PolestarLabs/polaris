...
const { name: N_YEP, reaction: R_YEP } = _emoji`yep`;
const { name: N_CHANNEL, reaction: R_CHANNEL } = _emoji`channel`;
const { name: N_SAVE, reaction: R_SAVE } = _emoji`save`;
const { name: N_UNDO, reaction: R_UNDO } = _emoji`undo`;
const { name: N_REDO, reaction: R_REDO } = _emoji`redo`;
const { name: N_WG, reaction: R_WG } = _emoji`switchpartialON`;
const { name: N_WO, reaction: R_WO } = _emoji`switchpartialPARTIAL`;
const { name: N_WR, reaction: R_WR } = _emoji`switchpartialOFF`;
const MINI_ON = _emoji`swon`;
const MINI_OFF = _emoji`swoff`;
...
async function init(msg) {
  ...
  const omsg = await msg.channel.send({ embed: { color: 0x22d, title: "Please wait", description: "Currently configuring everything..." } });
  ...
  for (const emoji of menuEmoijis) if (!Array.isArray(emoji) || emoji[0].length) await omsg.addReaction(Array.isArray(emoji) ? emoji[0] : emoji);
  ...
  function exit(inactive = false) {
    ...
    if (!Switch.saved) {
      msg.channel.send({ embed: { color: 0x33D, title: embedTitle, description: "Do you want to save your changes? (yes|no)" } }).then((savemsg) => {
        ...
        new Promise((res) => {
          setTimeout(() => res([]), 2 * INPUT_TIMEOUT);
          msg.channel.awaitMessages((m) => msg.author.id === m.author.id && /yes|no/.test(m.content), { maxMatches: 1, time: INPUT_TIMEOUT }).then(res);
          savemsg.awaitReactions((r) => msg.author.id === r.userID && [N_YEP, N_NOPE].includes(r.emoji.name), { maxMatches: 1, time: INPUT_TIMEOUT }).then(res);
        }).then((r) => {
...
function genSwitchEmbed(Switch, options) {
  ...
  const disable = options?.disable || false; // whether exited
  const inactive = options?.inactive || false;
  const cat = Switch.mode === "category" ? Switch.category : null; // current category if any
  const cmode = Switch.scope === "channel"; // channel mode
  ...
  if (!cat) {
    ...
    for (const cat of catsArr) {
      ...
      const override = cmode && (cats[cat].cmds.some((cmd) => cdcmds.includes(cmd) || cecmds.includes(cmd)));
    ...
    let fieldCount = 1;
    const cmdCount = cats[cat].cmds.length;
    if (cmdCount > 5) {
