...
const init = async function (msg){
        ...
        let prefilt = MRG.filter((v,i,a)=>{
            //let unique = i == a.map(x=>x.id).indexOf(v.id);
            let unique = a.map(x=>x.id).filter(x=>x==v.id).length === 1;
            if(!unique){
                let copies= a.filter(ff=>ff.id == v.id);                
                let oldest =  copies.sort((a,b)=> a.since - b.since )[0];
                copies.forEach(c=> c.ponderedValue = c.ring == 'stardust'?0:c.ring == 'sapphire'?1:c.ring == 'rubine'?2:c.ring == 'jade'?3:9 )
                let mostValuable = copies.sort((a,b)=> a.ponderedValue - b.ponderedValue )[0];
...
${x.preexistent? `PREEXISTENT: ${x.preexistent._id}\n` :''}`).join('')  }
    ...
    const responses = await msg.channel.awaitMessages(msg2 =>
        ...
        if ( action.startsWith('import') ) {
            ...
            let newM = await DB.relationships.create("marriage" , newMARRIAGES[subaction].users , newMARRIAGES[subaction].initiative ,newMARRIAGES[subaction].ring);
