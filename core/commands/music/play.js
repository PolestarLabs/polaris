...
const init = async function (msg) {
  setTimeout(async () => {
    const { voiceState } = await PLX.resolveMember(msg.guild.id, PLX.user.id).catch(() => {});
    if (!voiceState?.channelID) return;
    const currentChannel = await PLX.getChannel(voiceState.channelID);
    if (currentChannel.type !== 13) return;

    axios.patch(
      `https://discord.com/api/v8/guilds/${currentChannel.guild.id}/voice-states/${PLX.user.id}`,
      {
        suppress: false,
