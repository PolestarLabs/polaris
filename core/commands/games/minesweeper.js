...
const init = async (msg) => {
  ...
  for (i = 0; i < SQ; i++) {
    for (j = 0; j < SQ; j++) {
      if (arrGrid[i][j] === `||${BOMB}||`) continue;
      let around = 0;
      if ((arrGrid[i] || [])[j - 1] === `||${BOMB}||`) around += 1; // N
      if ((arrGrid[i] || [])[j + 1] === `||${BOMB}||`) around += 1; // S

      if ((arrGrid[i - 1] || [])[j] === `||${BOMB}||`) around += 1; // W
      if ((arrGrid[i + 1] || [])[j] === `||${BOMB}||`) around += 1; // E

      if ((arrGrid[i - 1] || [])[j - 1] === `||${BOMB}||`) around += 1; // NW
      if ((arrGrid[i - 1] || [])[j + 1] === `||${BOMB}||`) around += 1; // SW

      if ((arrGrid[i + 1] || [])[j - 1] === `||${BOMB}||`) around += 1; // NE
      if ((arrGrid[i + 1] || [])[j + 1] === `||${BOMB}||`) around += 1; // sE
