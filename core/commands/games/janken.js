...
const init = async function (msg, args) {
  ...
  const res = await prompt.awaitButtonClick((int) => {
    const res = participants.includes(int.userID);
  ...
  }, { removeButtons: false, maxMatches: 2, time: 30e3 }).catch((err) => msg.reply("No one choosed..."));

  prompt.edit({
    content: prompt.content,
    components: [{
      type: 1,
      components: rpsComponents.map((x) => {
        x.disabled = true;
        return x;
      }),
    }],
