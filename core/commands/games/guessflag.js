...
const init = async function (msg, args) {
  ...
  const Flags = new GG("guessflags", "image", {
  ...
  Flags.play(msg).then(async (results) => {
    if (results.score) {
      ...
      if (solo) {
        data = {
          id: `${msg.author.id}`, type: "guessflag-solo", points: results.score, timestamp: Date.now(), data: results,
        };
      } else {
        data = {
          id: `${msg.guild.id}`, type: "guessflag-server", points: results.score, timestamp: Date.now(), data: results,
        };
      }
