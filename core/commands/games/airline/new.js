...
const airlineNew = async (msg, args) => {
  ...
  const existent = await DB.airlines.AIRLINES.findOne({ id: airlineID });

  if (existent) return c1[0].reply("An airline with this ID already exists. Please try again");
