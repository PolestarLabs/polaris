...
const drawTable = async (PL, DL, DATA_A, DATA_B, drawOpts) => {
  ...
  wid = nameP.width > 100 * 2 ? wid = 100 * 2 : nameP.width;
  ...
  wid = nameD.width > 100 * 2 ? 100 * 2 : nameD.width;
...
const DECK = async (msg, args) => {
  ...
  if (targetDeck && USERDATA.modules.skinInventory.includes(targetDeck.id)) {
    ...
    let deckSwitchMessage = `${rand$t("responses.verbose.interjections.acknowledged")} `
      + `${$t("games.blackjack.switchdeck", P)} ${rand$t("responses.verbose.opinion_decks", P)}`;
...
const init = async (msg, args) => {
  ...
  if (Number.isNaN(bet)) { PLX.autoHelper("force", { cmd: "blackjack", msg, opt: "gambling" }); return null; }
  ...
  async function getFinalHand(blackjack, playerHand, dealerHand, deck, pUps, options) {
    let {
      m, intro: introMessage, B: balance, b, log: { usr: log },
    } = options;
    ...
    async function ProcessHand(currentHand) {
      ...
      if (currentHand.length === 1) blackjack.hit(currentHand, pUps);
      ...
      if (currentHand.insuredLastTurn) {
        m.channel.send(`${_emoji("plxbjkinsu")} **Insurance has been placed for this match!**`,
          { file: scenario.toBuffer("image/png", imageOptions), name: "blackjack.png" }).then((mm) => {
        // m.channel.send(`Insurance has been placed for this match!`).then(mm=>{
        ...
        m.channel.send("", { file: finalImage, name: "blackjack.png" }).then((mm) => {
          if (tableMessageRound) tableMessageRound.delete();
          tableMessageRound = mm;
        });
      }
      if (introMessage) {
        ...
        m.channel.send(hitstand).then((mm) => (introMessage = mm));
      }

      const responses = await m.channel.awaitMessages((msg2) => msg2.author.id === m.author.id && (
        msg2.content === "hit"
        || msg2.content === "stand"
        || msg2.content === "STANDO"
        || msg2.content === "stando"
      ...
      if (action === "insurance") {
        ...
        log.push(`> ${_emoji("plxbjkinsu")} \`INSURANCE\` (${Math.ceil(b / 2)})`);
      }
      if (action === "surrender") {
      ...
      if (action === "stando") {
        ...
        m.channel.send($t("eastereggs.konodioda", { lngs: m.lang }));
      }
    ...
    await ProcessHand(cHand);

    // introMessage.delete().catch(e=>null);
    tableMessageRound.delete().catch(() => null);
    return hands;
  }
  ...
  return msg.channel.send(hitstandPre).then(async (introMessage) => {
    ...
    if (playerHandValueCalc !== "Blackjack" && !playerHandValueCalc.toString().includes("JOKER")) {
      ...
      while ((Blackjack.isSoft(dealerHand) || Blackjack.handValue(dealerHand) < 17) && !noHit) {
        log.plx.push(`> ${Blackjack.handValue(dealerHand) > 21
          ? _emoji("plxbjkbust") : _emoji("plxcards")} Pollux \`HIT\` **${Blackjack.handValue(dealerHand)}** > `
    ...
    playerHands.forEach((hand, i) => {
      ...
      H_DATA.result = playerHands.length === 1
        ? `** ${msg.member.displayName}**` : ` ${result.replace(/(^\w|\s\w)/g, (ma) => ma.toUpperCase())}${result !== "push"
          ? `, ${lossOrGain}` : `, ${"Rubines"}"} back`}\n`;
      ...
      RESULT_EMOJI = (res) => (playerValue.toString().includes("JOKER")
        ? _emoji("plxbjkjkr")
        : playerValue === "Blackjack"
          ? _emoji("plxbjkbjk")
          : res === "push"
      ...
      splitExplain.push(`${_emoji("plxcards").no_space}\`\u200b${(`${i + 1}`).padStart(2, " ")}\` : `
        + `**\`\u200b${(`${lossOrGain}`).padStart(6, " ")}\`** ${_emoji("RBN")} ${RESULT_EMOJI(result)}${hand.doubled ? _emoji("plxbjk2x") : ""}`
    ...
    msg.channel.send(PLAY_RES,
      { file: scenario.toBuffer("image/png", imageOptions), name: "blackjack.png" }).then((m) => m.channel.send(rebalance).catch(() => null));
    if (splitExplain.length) {
      // msg.channel.send(`**${splitExplain.length>1
      //  ? $t('games.blackjack.splitbreak',P): $t('games.blackjack.result',P)}**\n`+splitExplain.join('\n'))
