...
const init = async function (msg, args) {
	...
	await ECO.pay(msg.author.id, parseInt(args[0]));
...
const startGameCollector = async (msg) => {
	...
	if (result.stopped) {
		...
		if (better) await ECO.pay(msg.author.id, result.currentValue, 'RUSSIANROULETTE');
	...
	if (result.lost) {
		...
		await ECO.receive(msg.author.id, parseInt(msg.args[0]) * 1.5, 'RUSSIANROULETTE');
