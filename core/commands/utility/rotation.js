...
const axios = require("axios").default; // Just for types
...
const init = async function (message) {
  ...
  if (args === "league of legends" || args === "league" || args === "lol") await lol();
  else if (args === "heroes of the storm" || args === "hots" || args === "hos" || args === "heroes") await hots();
  ...
  async function hots() {
    ...
    const req = await axios.get("http://heroesofthestorm.gamepedia.com/Free_rotation").catch(() => null);
...
`, true);
  ...
  async function lol() {
    const req = await axios.get(`https://na1.api.riotgames.com/lol/platform/v3/champion-rotations?api_key=${riot}`).catch(() => null);
    if (!req) return;
    const body = req.data.freeChampionIds;
    const body4new = req.data.freeChampionIdsForNewPlayers;
    const maxnewlv = req.data.maxNewPlayerLevel;
