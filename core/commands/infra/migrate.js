...
`;
  ...
  const newRubines = Math.min(~~((userData.modules.rubines || 0) * 0.05) + (userData.modules.dyStreakHard || 1) * 10, 50000);
  const oldRubines = userData.modules.rubines || 0;
  const jades      = ~~(userData.modules.jades / 2);
  const saph       = ~~(userData.modules.sapphires * (SAPPHIREFACTOR(userData.donator, userData.formerDonator)) / 10 + 1);

  await DB.users.set(msg.author.id, {
    $set:
        {
          "counters.daily.streak": userData.modules.dyStreakHard || 0,
          "counters.daily.last": userData.modules.daily || 0,
...
`;
  ...
  const myBGs = userData.modules.bgInventory;
