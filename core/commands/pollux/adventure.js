...
0                                                                                
    ...
    let tally = await msg.channel.send({embed:tallyEmbed});

    let res_LOC = await Screen(locationEmbed, LOCATIONS.map(x=>x.emoji) );
    //-----------------------------------------------
    let selectedLocation = LOCATIONS[res_LOC.res.index];
    //-----------------------------------------------
    delete embed.title
    ...
    ${INSURANCE_OPTS.join('\n')}`
    let res_INS = await Screen({embed}, INSURANCES );
    //-----------------------------------------------
    let selectedInsurance = INSURANCE_VALS[res_INS.res.index]
    //-----------------------------------------------
    delete embed.title
    ...
    ${TIME_OPTS.join('\n')}`
    let res_DUR = await Screen({embed}, TIMES);
    //-----------------------------------------------
    let selectedTime = res_DUR.res.index == 0 ? 1
    ...
    let Adventure = new Venture(userData,selectedTime,'desert');
    let journeyLog = new Journey(Adventure);
