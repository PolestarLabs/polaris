...
const init = async (msg, args) => {
  ...
  const [userData, Rings] = await Promise.all([DB.users.getFull({ id: msg.author.id }), DB.items.find({ series: "ring" })]);
  ...
  const mrgPresent = marriages.find((mrg) => mrg.users.includes(Target.id));
  if (mrgPresent) {
    const RING_B = determineRing(mrgPresent.ring, Rings);
    P.ringA = RING_B.name;
    P.ringB = RING.name;
    if (RING_B.misc.rank > RING.misc.rank || RING_B.id === RING.id) {
      return {
        embed: {
          color: 0xfc6858,
    ...
    const upgradePrompt = await msg.channel.send({
      embed: {
        color: 0xfc9d58,
    ...
    return YesNo(upgradePrompt, msg, () => {
...
function noRingResponse(Rings, userData, P) {
  ...
  if (ringCollection.length > 0) ringCollection = (`${$t("responses.marry.availableRings", P)}\n\n${ringCollection.join("")}`);
  else ringCollection = $t("responses.marry.actuallyNone", P);
  return {
    embed: {
...
async function upgradeMarriage(msg, args, userData, RING, RING_B, mrgPresent, upgradePrompt) {
  ...
  if (!(userData?.id === msg.author.id && RING && mrgPresent)) {
    [userData, Rings] = await Promise.all([DB.users.getFull({ id: msg.author.id }), DB.items.find({ series: "ring" })]);
