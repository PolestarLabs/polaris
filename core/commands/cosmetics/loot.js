...
async function composeItem(currentItem,base,UDATA){
  ...
  switch(currentItem.type){
    case "RUBINES":
    case "JADES":
      img =await Picto.getCanvas(paths.BUILD + "LOOT/" +(currentItem.emblem) + ".png");
  ...
  let rarity = await Picto.getCanvas(appRoot+"/../v7/dash/public/images/tiers/new/" + currentItem.rarity + ".png")
  let rarityShine = await Picto.getCanvas(paths.BUILD + "LOOT/shine" + currentItem.rarity + ".png")
...
async function drawBox(BOX,message){
  ...
  let LOOTS = []
    ...
    let sq = 70
    let shift = (50-sq)/2
    const B = await Picto.getCanvas(paths.BUILD + "LOOT/mainframe.png");
...
async function renderBG(bname,UDATA,Rar) {
    ...
    let J = await Picto.getCanvas(paths.BUILD+"LOOT/BG.png");
    let NW = await Picto.getCanvas(paths.BUILD+"new.png");
    ctx.rotate(0.16);
...
async function renderMedal(medal,UDATA,Rar) {  
    ...
    let J = await Picto.getCanvas(paths.BUILD+"LOOT/MEDAL.png");
    let NW = await Picto.getCanvas(paths.BUILD+"new.png");
...
async function renderSticker(stk) {
    ...
    let J = await Picto.getCanvas(paths.BUILD+"LOOT/STAMP.png");
    ctx.drawImage(I, 94, 128,100,100);
    ctx.drawImage(J, 26, 26);
  ...
  switch(iMETA.type){
    ...
    case "BOOSTER":
      ...
      let m=iMETA.rawName;
        ...
        DB.users.set(message.author.id,{$push:{'modules.inventory':m}}).then(ok=>{
          P.itm= iMETA.name
          return resolve(ej("no1")+$t("loot.addedSticker",P));
        }).catch(e=> resolve("ERROR"))
...
async function keeppit(LOOTS,boxaher,message,UDATA,rerollCost,thisRoll) {
  for (let i = 0; i < 3; i++) {
    ...
    if(thisRoll>0){
      ...
      await DB.items.consume(message.author,boxaher);
};
