...
async function isAllowed(user, amount) {
  ...
  return DCN.fetch(filter).then((transactions) => {
    ...
    if ((transactions.reduce((a, b) => a + parseFloat(b.amount), 0) + amount) / (1 - tax) > dailyMax) return { allowed: false, reason: "dailyMax" };
...
const init = async (msg, args) => {
  ...
  embed.fields.push({
    ...
    value: $t("responses.discoin.tax_desc", {
      ...P, exchange: "15", income: "12", transfer: "5",
    }),
    inline: true,
  });
  embed.fields.push({
  ...
  async function createCard(curr) {
    ...
    const perRBN = RBN.value / curr.value;
    const perRBNString = perRBN > 10 ? miliarize(perRBN) : perRBN.toPrecision(2).replace(".", ","); // not actually a string
