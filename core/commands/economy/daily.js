...
const init = (msg) => {
  ...
  const after = async (message, Dly) => {
    ddy = await Dly.userData(message.author);
    ...
    DB.users.getFull({ id: Author.id }).then(async (userData) => {
      ...
      // minibuster.up(msg,hardStreak+softStreak*10)
      ...
      if (hardStreak >= 10 && !userachinv.includes("10daily")) {
        ...
        msg.reply("**New Achievement!** Clockwork Collector | Collected 10 Dailies");
      }
      if (hardStreak >= 30 && !userachinv.includes("30daily")) {
        ...
        msg.reply("**New Achievement!** Collector of the Month | Collected 30 Dailies");
      }
      if (hardStreak >= 100 && !userachinv.includes("100daily")) {
        ...
        msg.reply("**New Achievement!** Compulsive Collector | Collected 100 Dailies");
      }
      if (hardStreak >= 200 && !userachinv.includes("200daily")) {
        ...
        msg.reply("**New Achievement!** Blood for a silver Sapphire | Collected 200 Dailies");
      }

      if (userData.spdaily && userData.spdaily.amt) {
        embed.addField(Author.dDATA.spdaily.title, `+${Author.dDATA.spdaily.amt}${_emoji("rubine")}`);
        await ECO.receive(msg.author.id, Author.dDATA.spdaily.amt, "special_daily_boost", "RBN", "+");
      }
  ...
  const info = async (message, Daily) => {
    const userDaily = await Daily.userData(message.author);
    const dailyAvailable = await Daily.dailyAvailable(message.author);
    const streakGoes = await Daily.keepStreak(message.author);
