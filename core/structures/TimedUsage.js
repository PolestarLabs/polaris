...
class DailyCmd {
  ...
  async parseUserData(user) {
    ...
    const userDaily = USERDATA.counters?.[this.command] || {
      last: 1,
      streak: 1,
    };
  ...
  async streakProcess(streakContinues, user) {
    if (streakContinues) {
      if (this.userDaily.streak > (this.userDaily.highest || 1)) {
        await DB.users.set(user.id, {
          [`counters.${this.command}.highest`]: this.userDaily.streak,
        });
      }
    ...
    if (this.userDaily.insured) {
      await DB.users.set(user.id, {
        [`counters.${this.command}.insured`]: false,
      });
    ...
    await DB.users.set(user.id, {
      [`counters.${this.command}.lastStreak`]: this.userDaily.streak,
    });
...
exports.init = async function init(
  ...
  if (Author.dailing === true) {
    Author.dailing = false;
    return message.channel.send(
      `There's already a \`${Daily.command}\` request going on!`
    );
  }
  ...
  if (
    message.args.includes("status") ||
    message.args.includes("stats") ||
    message.args.includes("info")
  ) {
    ...
    embe2.description(`
${_emoji("time")} ${_emoji("offline")} **${v.last}** ${formatDistanceToNow(
      userDaily
    )}
${_emoji("future")} ${dailyAvailable ? _emoji("online") : _emoji("dnd")} **${
      v.next
    }** ${formatDistanceToNow(addHours(userDaily, DAY / 1000 / 60 / 60))}
  `);
  ...
  try {
    await wait(0.2);
    const now = Date.now();
    DB.users.set(Author.id, {
      $set: { [`counters.${Daily.command}.last`]: now },
    });
    ...
    if (Daily.streak && streakStatus !== "lost") {
      ...
      await DB.users.set(Author.id, {
        $inc: { [`counters.${Daily.command}.streak`]: 1 },
      });
    } else if (Daily.streak && streakStatus === "lost") {
      await DB.users.set(Author.id, {
        $set: { [`counters.${Daily.command}.streak`]: 1 },
      });
    }
    Daily.streakStatus = streakStatus;

    end();
    await success(message, Daily);
    // DEBUG_LOG();
    return null;
  } catch (e) {
    end();
