...
exports.run = (msg, args, userData, itemDetails) => new Promise(async (resolve) => {
  ...
  if ((destinationCounter.highest || 0) <= destinationCounter.streak) return msg.channel.send("You cannot restore your streak to a number below your current one.");

  const promptMessage = await msg.channel.send({
    embed: {
      description: "Would you like to consume your Streakfix ?" + `
\`${TARGET}\` [${destinationCounter.streak}] >> [${destinationCounter.highest}]        
        `,
    },
  });
  ...
  YesNo(promptMessage, msg,
    async (cancel, prompt) => {
      ...
      await (await DB.userInventory.getFull(userData.id)).removeItem("streakfix+", 1);
