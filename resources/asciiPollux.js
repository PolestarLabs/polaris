...
const CLIENT_DATA         = JSON.parse(process.env.PRIME_FLAVORED_CLIENT);

const UNIT = isPRIME ? `⭐ PRIME (${CLIENT_DATA.name})` : ` ${CLUSTER_NAMES[CLUSTER_ID]} `.bgGreen || `Cluster ${CLUSTER_ID}`;
...
const ascii = function () {
  ...
  const text = `${"                                                                "[a][b]
    ...
    }\n${" Yb,  88      `8b      IP'`Yb IP'`Yb    "[a][b]+`${UNIT}`.magenta
